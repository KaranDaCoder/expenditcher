...
const EditAnExpense = ({ owner_id, data, exp_detail }) => {
 ...
 const [expense, setExpense] = useState({
  category: exp_detail?.result?.category || 'Other Services',
  name: exp_detail?.result?.name || '',
  amount: exp_detail?.result?.amount/100 || '',
  state: exp_detail?.result?.state || 'MN',
  payment_mode_id: exp_detail?.result?.payment_mode_id?._id || data?.result[0]?._id,
  date: new Date(exp_detail?.result?.date).toDateString().toString() || new Date().toLocaleDateString(),
  status: exp_detail?.result?.status ||  'completed',
  desc: exp_detail?.result?.desc || '',
  owner_id: owner_id
 })
 ...
 const handleAddExpenseRequest = async (e) => {
  e.preventDefault();
  try {
   const request = await fetch(`/api/expenses/${exp_detail?.result?._id}`, { method: 'PATCH', cache: 'no-cache', redirect: 'follow', body: JSON.stringify(expense) });
   ...
   >
    ...
    <div className='flex flex-row justify-around w-full gap-2 mt-4'>
     ...
     <button
      ...
      onClick={() => { router.replace(`/expenses/${exp_detail?.result?._id}`) }}
     >
      Edit
     </button>
    </div>
