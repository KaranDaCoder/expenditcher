...
const AddExpenseForm = ({ allCategories }) => {
  ...
  const [expenseObject, setExpenseObject] = useState({
    ...
    date: expenseDate,
    state: null,
    // payment_mode: allPaymentModes?.allPaymentModes[0]._id || '',
    status: 'Completed',
    desc: null,
  });
      ...
      <div className='flex flex-col items-center w-full h-full lg:space-y-8'>
        <div className='flex flex-col items-center justify-center w-full'>
          ...
          <select
            name='category'
            id='category'
            className='w-full py-1 text-base font-semibold text-center border rounded-md outline-none lg:w-5/6 text-slate-600'
        ...
        <div className='flex flex-col items-center justify-center w-full'>
          ...
          <select
            name='payment_mode'
            id='payment_mode'
            className='relative w-full py-1 text-lg font-semibold text-center border rounded-md outline-none lg:w-5/6 text-slate-600'
            // onChange={handleFormSubmission}
          >
            {/* {allPaymentModes?.allPaymentModes?.map((mode) => (
          ...
          <Link
            href={'/manage'}
            className='mb-2 text-xs font-semibold underline w-fit text-slate-800 hover:text-fuchsia-900'
          >
