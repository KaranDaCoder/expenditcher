  ...
  return (
    <div className='relative w-full h-auto p-2'>
      ...
      {data?.count > 0 ?  <table className='w-full table-fixed'>
        <thead className='sticky top-0 z-0 text-white'>
        ...
        <tbody className='flex flex-col'>
          {data?.result.map(expense => (
            <tr className='flex items-center justify-between w-full px-2 mt-2 mb-2 text-sm transition-all duration-500 border rounded-md shadow-md cursor-pointer lg:text-base min-h-20 hover:font-semibold' key={expense._id}>
