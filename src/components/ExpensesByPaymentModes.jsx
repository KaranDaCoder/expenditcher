...
const ExpensesByPaymentModes = async ({ data }) => {
 ...
 return (
  <div className='col-span-1 row-start-2 bg-white border min-h-60 rounded-xl hover:border hover:border-green-500'>
   ...
   <div className='flex flex-col items-start justify-start w-full h-full px-10 py-4 space-y-4 text-sm capitalize lg:text-base'>
    {paymentModes?.result?.map(async paymentMode => (
     <div className='flex items-center justify-between w-full gap-2' key={paymentMode?._id}>
